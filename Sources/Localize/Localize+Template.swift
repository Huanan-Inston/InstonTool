...
extension Localize {
    struct Template: AsyncParsableCommand {
        static let configuration = CommandConfiguration(
            abstract: "Templating iOS project Strings by Jinja",
            usage: """
            tool localize template <template>... --output <output> --strings <strings>
            
            Example:
                tool localize template ./LangGen/OCL+Keys.h.template ./LangGen/OCL+Keys.m.template --output ./LangGen --strings ./MapRunner/Strings
            """,
            discussion: """
            """
        )
        ...
        @Argument(
            help: .init(
                "The file paths of the templates. ",
                discussion: """
